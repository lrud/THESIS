...
\section{Baseline Model Specifications}

\subsection{Model Comparison Target}

All baseline models forecast next-period DVOL change:
\begin{equation}
\Delta y_{t+1} = y_{t+1} - y_t
\end{equation}

\subsection{OLS Baseline (All Features)}

\textbf{Model:} Linear regression with standardized features
\begin{equation}
\Delta \hat{y}_{t+1} = \beta_0 + \sum_{i=1}^{8} \beta_i x_{t,i}
\end{equation}

\textbf{Features (8):} $y_t$, $y_{t-1}$, $y_{t-7}$, $y_{t-30}$, transaction volume, network activity, NVRV, DVOL-RV spread

\subsection{HAR-RV Volatility Focused}

\textbf{Model:} HAR-RV with only volatility lag features
\begin{equation}
\Delta \hat{y}_{t+1} = \beta_0 + \beta_d \cdot y_{t-1} + \beta_w \cdot \bar{y}_{t-7:t-1} + \beta_m \cdot \bar{y}_{t-30:t-1}
\end{equation}

\textbf{Features (3):} $y_{t-1}$ (daily lag), $\bar{y}_{t-7:t-1}$ (weekly average), $\bar{y}_{t-30:t-1}$ (monthly average)

\subsection{Random Forest Baseline}

\textbf{Model:} Ensemble of decision trees
\begin{equation}
\hat{y}_{t+1} = \frac{1}{B}\sum_{b=1}^{B} f_b(X_t)
\end{equation}

\subsection{XGBoost Baseline}

\textbf{Model:} Gradient boosting with trees
\begin{equation}
\hat{y}_{t+1} = \sum_{k=1}^{K} f_k(X_t), \quad f_k \in \mathcal{F}
\end{equation}
