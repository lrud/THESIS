...
def train_unified(
    model_type, config, save_prefix="cli", results_dir="results/cli_training"
):
    ...
    model, device, effective_batch_size, param_count, use_multi_gpu = (
        setup_device_and_model(config, input_size)
    )
    ...
    for epoch in range(config["epochs"]):
        ...
        if (epoch + 1) % 2 == 0 or epoch == 0:
            ...
            print(
                f"Epoch {epoch + 1:3d}/{config['epochs']} | Train: {train_loss:.6f} | Val: {val_loss:.6f} | "
                f"LR: {optimizer.param_groups[0]['lr']:.2e} | Time: {elapsed / 60:.1f}m"
            )

            with open(f"results/logs/current_training.log", "a") as f:
                f.write(
                    f"{datetime.now().isoformat()} | Epoch {epoch + 1:3d}/{config['epochs']} | "
