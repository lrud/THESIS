...
FEATURE_SETS = {
    "no_lags": ["transaction_volume", "network_activity", "nvrv", "dvol_rv_spread"],
    "no_lags_jumps": [
        "transaction_volume",
        "network_activity",
        "nvrv",
...
class UnifiedLSTMDataset(Dataset):
    ...
    def __init__(
        self, data, feature_set, sequence_length=24, window_size=720, mode="train"
    ):
        self.data = data.copy()
    ...
    def _apply_rolling_normalization(self):
        """Apply rolling window normalization to all features and target."""
        for col in self.feature_cols:
            ...
            rolling_mean = (
                self.data[col].rolling(self.window_size, min_periods=1).mean()
            )
            rolling_std = self.data[col].rolling(self.window_size, min_periods=1).std()
            rolling_std = rolling_std.replace(0, 1e-8)
            self.data[f"{col}_normalized"] = (
                self.data[col] - rolling_mean
            ) / rolling_std

        target_rolling_mean = (
            self.data[self.target_col].rolling(self.window_size, min_periods=1).mean()
        )
        target_rolling_std = (
            self.data[self.target_col].rolling(self.window_size, min_periods=1).std()
        )
        target_rolling_std = target_rolling_std.replace(0, 1e-8)
        self.data["target_normalized"] = (
            self.data[self.target_col] - target_rolling_mean
        ) / target_rolling_std
    ...
    def _prepare_sequences(self):
        ...
        normalized_features = [
            f"{col}_normalized" if col not in JUMP_INDICATORS else col
            for col in self.feature_cols
        ]

        for i in range(self.sequence_length, len(self.data) - 1):
            X_seq = (
                self.data[normalized_features].iloc[i - self.sequence_length : i].values
            )
            X_list.append(X_seq)
            ...
            if self.use_weighting:
                is_jump = self.data["lee_mykland_jump"].iloc[i + 1]
...
def create_unified_dataloaders(
    ...
    train_dataset = UnifiedLSTMDataset(
        train_data, feature_set, sequence_length, window_size, "train"
    )
    val_dataset = UnifiedLSTMDataset(
        val_data, feature_set, sequence_length, window_size, "val"
    )
    test_dataset = UnifiedLSTMDataset(
        test_data, feature_set, sequence_length, window_size, "test"
    )
