#!/bin/bash
# =============================================================================
# Single GPU Retraining - Learning Rate Matched to Multi-GPU
# Purpose: Confirm that learning rate (not DataParallel) is the key factor
# Strategy: Use exact same parameters as multi-GPU except use_single_gpu=false
# =============================================================================
...
echo "========================================"
echo "Single GPU Training - LR Matched to Multi-GPU"
echo "========================================"
echo "Date: $(date)"
echo "Purpose: Confirm learning rate hypothesis (LR=5e-05)"
echo ""

# Create results directory
RESULTS_DIR="results/single_gpu_lr_matched_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"
...
# =============================================================================
# Training Configuration - EXACT MATCH TO MULTI-GPU
# =============================================================================

# Multi-GPU used: --lr 0.0001 --use-multi-gpu -> actual LR = 5e-05
# We will use: --lr 0.00005 (directly specify the scaled LR)

EPOCHS=100
BATCH_SIZE=32
LR=0.00005  # 5e-05 - matches multi-GPU actual learning rate
PATIENCE=10  # Matches multi-GPU patience (not 15!)
...
train_model() {
    ...
    echo "----------------------------------------"
    echo "Configuration:"
    echo "  - Learning Rate: $LR (matches multi-GPU)"
    echo "  - Patience: $PATIENCE (matches multi-GPU)"
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID .venv/bin/python cli/bin/train.py "$MODEL_TYPE" \
        ...
        --epochs "$EPOCHS" \
        --patience "$PATIENCE" \
        --save-prefix "lr_matched_${MODEL_NAME}"

    # Copy result to our results directory
    LATEST_RESULT=$(ls -t results/cli_training/*_*_${MODEL_TYPE}_*.json 2>/dev/null | head -1)
...
echo "Performance Summary (Single GPU with Matched LR):"
echo "---------------------------------------------------"
...
echo ""
echo "========================================"
echo "Comparison with Multi-GPU Results"
echo "========================================"
echo ""
echo "Expected results (should match multi-GPU):"
echo "  - market_lags (512x7): R² ≈ 0.8021, RMSE ≈ 2.85"
echo "  - jump_aware (512x7): R² ≈ 0.800, RMSE ≈ 2.86"
echo "  - market (256x3): R² ≈ 0.6686, RMSE ≈ 3.68"
echo ""
echo "Previous single-GPU results (LR=0.0001, WRONG):"
echo "  - market_lags (512x7): R² = 0.6672, RMSE = 3.69"
echo "  - jump_aware (512x7): R² = 0.6679, RMSE = 3.69"
echo ""
